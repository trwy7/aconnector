...
def create_app_page():
    if request.user.disable_app_create:
        return render_template("templates/error.html", status_code=403, error_message="An administrator has restricted your account from creating apps. Contact the site administrator for more information."), 403
    da = App.create(
        request.user,
        "My new app"
    )
